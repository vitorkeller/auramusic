...
export default function PlaylistsPage() {
  ...
  const availableSongs = [
    { id: 1, title: "Midnight City", artist: "M83", cover: "from-blue-600 to-indigo-900", duration: "4:03" },
    { id: 2, title: "Starboy", artist: "The Weeknd", cover: "from-red-600 to-red-900", duration: "3:50" },
    { id: 3, title: "Instant Crush", artist: "Daft Punk", cover: "from-orange-500 to-amber-700", duration: "5:38" },
    { id: 4, title: "Resonance", artist: "HOME", cover: "from-purple-600 to-pink-900", duration: "3:32" },
    { id: 5, title: "Goth", artist: "Sidewalks and Skeletons", cover: "from-slate-700 to-slate-900", duration: "3:10" },
    { id: 6, title: "Life in a bubble", artist: "Código & Café", cover: "from-slate-600 to-slate-900", duration: "3:45" },
  ];
  ...
  const handleCreatePlaylist = () => {
    ...
    const songsToSave = availableSongs.filter(s => selectedSongs.includes(s.id));

    const newPlaylist = {
      id: Date.now(),
      title: newPlaylistName,
      tracks: songsToSave.length,
      songs: songsToSave,
  ...
  const confirmDeletion = () => {
    ...
    if (confirm(`Tem certeza que deseja excluir ${playlistsToDelete.length} playlist(s)?`)) {
      const updatedPlaylists = playlists.filter(p => !playlistsToDelete.includes(p.id));
