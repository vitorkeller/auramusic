...
export default function Home() {
		...
		const fetchTracks = async () => {
			try {
				...
				if (response.ok) {
					...
					if (data.length > 0) {
						setTopCharts(data.slice(0, 3));
						setNewReleases(data.slice(3));
					}
				}
			} catch (error) {
	...
	const toggleLike = async (e: React.MouseEvent, song: any) => {
		e.stopPropagation();
		const token = localStorage.getItem("token");
		...
		try {
			if (isLiked) {
				await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/favorites/${song.id}`, {
					method: "DELETE",
					headers: { "Authorization": `Bearer ${token}` }
				});
			...
			<div className="p-8 pb-10 flex flex-col gap-12">
				<div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-8">
					...
					<div className="flex flex-col gap-6">
						...
						{topCharts.length === 0 ? (
							<p className="text-gray-500 text-sm">Adicione músicas no banco de dados.</p>
						) : (
							<div className="flex flex-col gap-4">
								{topCharts.map(item => {
									...
									const isImage = item.cover?.startsWith('http');

									return (
										<div
											key={item.id}
											onClick={() => handleSongClick(item, topCharts)}
											className={`p-4 rounded-xl flex items-center gap-4 cursor-pointer transition border border-white/5 group/card ${isThisSongPlaying ? "bg-white/10 border-blue-500/30" : "bg-[#121212] hover:bg-white/5"
												}`}
										>
											<div
												className={`w-14 h-14 rounded-lg flex-shrink-0 border border-white/5 flex items-center justify-center bg-cover bg-center ${!isImage ? `bg-gradient-to-br ${item.cover}` : ''}`}
												style={isImage ? { backgroundImage: `url(${item.cover})` } : {}}
											>
												{isThisSongPlaying && isPlaying && <Pause size={20} className="text-white fill-current shadow-black drop-shadow-md" />}
											</div>
				...
				<div className="flex flex-col gap-6">
					...
					{newReleases.length === 0 ? (
						<p className="text-gray-500 text-sm">Adicione mais músicas no banco de dados para ver aqui.</p>
					) : (
						<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-6">
							{newReleases.map((album) => {
								...
								const isImage = album.cover?.startsWith('http');

								return (
									<div
										key={album.id}
										onClick={() => handleSongClick(album, newReleases)}
										className={`backdrop-blur-md p-4 rounded-3xl transition-all cursor-pointer border flex flex-col group ${isThisSongPlaying
											? "bg-white/10 border-blue-500/40"
											: "bg-white/5 border-transparent hover:bg-white/10 hover:border-white/10"
											}`}
									>
										<div
											className={`aspect-square rounded-2xl mb-4 shadow-lg overflow-hidden relative bg-cover bg-center ${!isImage ? `bg-gradient-to-br ${album.cover}` : ''}`}
											style={isImage ? { backgroundImage: `url(${album.cover})` } : {}}
										>
											...
											<button
												...
												className={`absolute top-2 right-2 w-8 h-8 flex items-center justify-center rounded-full bg-black/40 backdrop-blur-md transition-all z-20 hover:scale-110 ${isLiked ? "opacity-100" : "opacity-0 group-hover:opacity-100"
													}`}
											>
												<Heart size={16} className={isLiked ? "fill-pink-500 text-pink-500" : "text-white"} />
											</button>

											<button className={`absolute bottom-2 right-2 w-10 h-10 flex items-center justify-center rounded-full bg-blue-500 text-black shadow-xl transition-all z-10 ${isThisSongPlaying ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0"
												}`}>
